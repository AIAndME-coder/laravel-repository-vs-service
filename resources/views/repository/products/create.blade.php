...
@section('title', 'Create Product - Repository Approach')

@section('content')
<div class="tw:flex tw:justify-between tw:items-center tw:mb-6 tw:pb-5 tw:border-b tw:border-gray-200">
    <div>
        <h1 class="tw:text-3xl tw:font-bold tw:text-gray-900">Create Product <span class="tw:inline-flex tw:items-center tw:px-3 tw:py-1 tw:rounded-full tw:text-xs tw:font-semibold tw:bg-green-100 tw:text-green-800">Repository Pattern</span></h1>
...
<div class="tw:bg-white tw:shadow-md tw:rounded-lg tw:p-6 sm:tw:p-8">
    <form action="{{ route('repository.products.store') }}" method="POST" class="tw:space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                      class="tw:w-full tw:px-4 tw:py-2.5 tw:border tw:border-gray-300 tw:rounded-lg focus:tw:outline-none focus:tw:ring-2 focus:tw:ring-green-500 focus:tw:border-transparent tw:transition-all tw:resize-vertical @error('description') tw:border-red-500 @enderror">{{ old('description') }}</textarea>
            @error('description')
                <p class="tw:mt-2 tw:text-sm tw:text-red-600 tw:flex tw:items-center">
                    <svg class="tw:w-4 tw:h-4 tw:mr-1" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
