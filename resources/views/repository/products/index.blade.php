...
<div class="tw:flex tw:justify-between tw:items-center tw:mb-6 tw:pb-5 tw:border-b tw:border-gray-200">
    <div>
        <h1 class="tw:text-3xl tw:font-bold tw:text-gray-900">Products <span class="tw:inline-flex tw:items-center tw:px-3 tw:py-1 tw:rounded-full tw:text-xs tw:font-semibold tw:bg-green-100 tw:text-green-800">Repository Pattern</span></h1>
        <p class="tw:mt-2 tw:text-sm tw:text-gray-600">Manage products using repository pattern</p>
    </div>
    <a href="{{ route('repository.products.create') }}" class="tw:inline-flex tw:items-center tw:px-4 tw:py-2 tw:bg-green-600 tw:text-white tw:font-semibold tw:rounded-lg hover:tw:bg-green-700 tw:transition-colors tw:shadow-sm">
        ...
        <tbody class="tw:bg-white tw:divide-y tw:divide-gray-200">
            @forelse($products as $product)
                <tr class="hover:tw:bg-gray-50 tw:transition-colors">
                    ...
                    <td class="tw:px-6 tw:py-4 tw:whitespace-nowrap tw:text-sm tw:font-bold tw:text-green-600">${{ number_format($product->price, 2) }}</td>
                    <td class="tw:px-6 tw:py-4 tw:whitespace-nowrap tw:text-right tw:text-sm tw:font-medium">
                        <div class="tw:flex tw:justify-end tw:gap-2">
                            <a href="{{ route('repository.products.show', $product) }}" class="tw:inline-flex tw:items-center tw:px-3 tw:py-1.5 tw:bg-gray-100 tw:text-gray-700 tw:text-xs tw:font-semibold tw:rounded hover:tw:bg-gray-200 tw:transition-colors">
                            ...
                            <a href="{{ route('repository.products.edit', $product) }}" class="tw:inline-flex tw:items-center tw:px-3 tw:py-1.5 tw:bg-blue-600 tw:text-white tw:text-xs tw:font-semibold tw:rounded hover:tw:bg-blue-700 tw:transition-colors">
                                <svg class="tw:w-4 tw:h-4 tw:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @empty
                <tr>
                    <td colspan="5" class="tw:px-6 tw:py-16 tw:text-center">
                        ...
                        <div class="tw:mt-6">
                            <a href="{{ route('repository.products.create') }}" class="tw:inline-flex tw:items-center tw:px-4 tw:py-2 tw:bg-green-600 tw:text-white tw:text-sm tw:font-semibold tw:rounded-lg hover:tw:bg-green-700">
                                <svg class="tw:w-5 tw:h-5 tw:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                </svg>
                                Create Product
